...
The app is not fully functional. Here are the known bugs:
* The submit button causes an internal server error. It's unknown whether any information is actually saved to the database.
* The display button does nothing. It won't even console.log the fact that it's been clicked.
* The tests of database methods appear to be working, as do the database methods themselves, but running the test will clear the existing database.
* At its current functionality, I expect a 60% on this project.

The app would also be better with a few improvements:
